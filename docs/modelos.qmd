...
# Concept DOI — always resolves to the newest version on Zenodo. Quarto renders
# this in the HTML title block and as a citation_doi meta tag (Google Scholar).
doi: 10.5281/zenodo.21361536
bibliography: bibliography.bib
# MAINTAINER NOTE — this page mirrors the working paper kept in the
# `manuscript-modelos` project. To refresh it: copy modelos.qmd over this file,
# then re-apply the site-only additions a plain copy would destroy:
#   1. `csl: apa.csl` below       (the manuscript project supplies this via its
#                                  own _quarto.yml, which does not apply here)
#   2. `execute: freeze: true`    (CI installs no R; the ENSO chunk must not run)
#   3. `other-links` below        (points at the pre-built Typst PDF)
#   4. these comments
#   5. the "Cite this work" callout in the body
# Then re-render locally with R so _freeze/docs/modelos/ refreshes, and commit
# _freeze along with the page.
#
# The PDF is NOT declared as a `format:` here, unlike the sibling SC13 pages.
# Two reasons: freeze is stored per-format (only html.json exists, so a pdf
# format would make CI execute R and fail), and a LaTeX render would look
# nothing like the Typst PDF archived on Zenodo. Instead docs/modelos.pdf is
# copied from manuscript-modelos/_manuscript/modelos.pdf — the same file as the
# deposit — and linked below. Refresh it whenever the paper changes.
other-links:
  - text: PDF
    href: modelos.pdf
    icon: file-pdf
csl: apa.csl
# The ENSO figure is an R chunk and CI has no R. Frozen results are committed
# under _freeze/, so the site renders this page without executing anything.
# Re-render locally (with R) after editing to refresh the freeze.
execute:
  freeze: true
---
...
Quiroz, J.-C., & Gozzer, R. (2026). *Reconciling competing stock assessments for
jumbo flying squid through probabilistic risk analysis*. Zenodo.
<https://doi.org/10.5281/zenodo.21361536>

The DOI above is the **concept DOI**: it always resolves to the most recent
version. To pin this exact deposit instead, cite
[10.5281/zenodo.21361537](https://doi.org/10.5281/zenodo.21361537).
:::
...
: Comparative overview of the three SPRFMO jumbo squid assessments. {#tbl-comparison tbl-colwidths="[16,28,28,28]"}
...
band_cols <- setNames(c("#f1c0b4", "#e9ecef", "#bcd6ef"), c(el_nino, "Neutral", la_nina))
pt_cols   <- setNames(c("#c0392b", "#7f8c8d", "#2e6da4"), c(el_nino, "Neutral", la_nina))
...
ggplot(ann, aes(Year, ICEN)) +
  ...
  scale_fill_manual(values = band_cols, name = "ENSO regime",
                    drop = FALSE) +
